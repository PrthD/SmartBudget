...
router.get('/', requireAuth, async (req, res, _next) => {
  ...
  try {
    const doc = await SavingsGoal.findOne({ user: req.userId });
    if (!doc) {
...
router.post(
  '/',
  requireAuth,
  validateSavingsDistribution,
  async (req, res, _next) => {
    logger.info(
      'POST /api/savings-goal - Creating new savings distribution doc'
    );
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Failed to create distribution doc' });
    }
  }
);
    ...
    try {
      ...
      if (doc.user.toString() !== req.userId) {
        return res.status(403).json({
          error: 'Not authorized to edit this savings distribution doc',
        });
      }
      ...
      return res
        .status(500)
        .json({ error: 'Failed to update distribution doc' });
    }
  }
);
...
router.delete('/:id', requireAuth, async (req, res, _next) => {
  ...
  try {
    ...
    logger.info('Distribution doc deleted successfully:', deleted);
    return res
      .status(200)
